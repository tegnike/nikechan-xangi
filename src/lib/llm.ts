...
export interface KarakuriDecision {
  command: string; // move / action / wait / conversation-start / transfer / info commands / etc.
...
**重要な制約**:
- 「有効な約束・予定（通常記憶より優先）」に pending の予定がある場合、それを通常の探索・読書・待機より優先してください。期限が近い、または過ぎている約束がある時は、move が選べるなら目的地 node へ直接移動してください。
- 約束時刻をまたぐ長時間 action / wait は選ばないでください。場所や node が不明な場合は map / status などで確認し、確認後は約束に向かってください。
...
{
  "command": "コマンド名（move/action/use-item/wait/transfer/transfer-accept/transfer-reject/conversation-start/conversation-accept/conversation-reject/conversation-join/conversation-speak/conversation-end/conversation-leave/conversation-stay/perception/actions/map/world-agents/status/nearby-agents/active-conversations/event）",
...
- transfer: args="agent-xyz --item apple --quantity 1", message=null
- transfer-accept: args="", message=null
- transfer-reject: args="", message=null
- conversation-start: args="agent-xyz", message="こんにちは！"
- conversation-speak: args="agent-xyz", message="なるほどですね。"（譲渡同梱時は args="agent-xyz --item apple --quantity 1" など）
- conversation-end: args="agent-xyz", message="またお話しましょう。"（譲渡応答時は args="agent-xyz --accept" など）
...
- perception: args="", message=null
- actions: args="", message=null
- map: args="", message=null
- world-agents: args="", message=null
- status: args="", message=null
- nearby-agents: args="", message=null
- active-conversations: args="", message=null
...
function guardInfoLoop(decision: KarakuriDecision, lastCommand: string): KarakuriDecision {
  const command = normalizeInfoCommand(decision.command);
  const previous = normalizeInfoCommand(lastCommand);
  if (INFO_COMMANDS.has(command) && INFO_COMMANDS.has(previous)) {
    console.warn(
...
function normalizeInfoCommand(command: string): string {
